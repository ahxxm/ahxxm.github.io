...
Append following to `/etc/lightdm/lightdm.conf`(find `[Seat: *]` part!):

    session-setup-script=xrandr --dpi 200

Or you can create a startup script, for example `/home/ahxxm/startup.sh`:

    #!/bin/bash
    xrandr --dpi 200

`chmod +x ~/startup.sh` and point session-setup-script to it:

    session-setup-script=/home/ahxxm/startup.sh
...
Follow author's [post](https://bbs.archlinux.org/viewtopic.php?id=162098) to setup Infinality, then everything will be automatically better.
...
- [ ] i3status items: ignore Ethernet and ipv6? refresh frequency
- [ ] Screenshot!
...
- [ ] better touchpad: two finger back is quite useful in browser, and improve accuracy? https://github.com/bulletmark/libinput-gestures
...
Append this to `~/startup.sh` to make it auto load:

    # since lightdm runs this as root
    su -ahxxm -c "ibus-daemon -drx"
