...
Then append following to `~/.config/i3/config`:
    ...
    bindsym F11 exec --no-startup-id maim --noopengl --select --format png /dev/stdout | xclip -selection clipboard -t image/png -i
    bindsym Print exec --no-startup-id maim --noopengl --select ~/Screenshots/$(date +%F-%T).png
